...
class NewtonMultivar :
  ...
  def __init__(self,fenetre9):
        ...
        #==============self.frame1  ===============================================#
        self.frame1 = Frame( self.fenetre9, bg="#88A780", highlightbackground="#182350",highlightthickness=2)
        self.frame1.place(x=48,y=30, height=660,width=900)
        #===============title=================================================#
        title = Label( self.fenetre9,text=" Newthon ",font=("Comic Sans MS", 17,"bold"), bg="#88A780",fg="#DCE5D9").place(x=390,y=5)
          ...
          # try:
                if(self.valop.get() == 2 ):
                    ...
                    if( self.valdespas.get()=="Optimal"):
                                if(verifierDP(hessien(xo))):
                                    while(normed > delta):
                                        ...
                                        print("le pas optimal dans l'itération  "+str(k)+"  est : ", alpha)
                                        xk = xo + alpha * directionk
                                        xo = np.array(xk).flatten()
                    ...
                    if(val3 == ""):
                        ...
                        dervSecX1 = sp.diff(fonction(xd),x1,x1)
                        dervSecX1X2 = sp.diff(fonction(xd),x1,x2)
                        dervSecX1X3 = sp.diff(fonction(xd),x1,x3)
                        dervSecX2X1 = sp.diff(fonction(xd),x2,x1)
                        dervSecX2 = sp.diff(fonction(xd),x2,x2)
                        dervSecX2X3 = sp.diff(fonction(xd),x2,x3)
                        dervSecX3X1 = sp.diff(fonction(xd),x3,x1)
                        dervSecX3X2 = sp.diff(fonction(xd),x3,x2)
                        dervSecX3 = sp.diff(fonction(xd),x3,x3)
